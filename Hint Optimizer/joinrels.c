...
make_join_rel(PlannerInfo *root, RelOptInfo *rel1, RelOptInfo *rel2)
{
	 ...
	 */
         ...
         elog(DEBUG1, "JoinRels: Entering forcejoin list");

         /*Go through the forcejoins list, if matches, store the information in joinrel*/
         for (p1 = list_head(root->glob->forcedjoins); p1 != NULL; p1 = p1_next)
       	{   
                elog(DEBUG1, "JoinRels: in the list");
//		ForcedJoin	   *fj;
 //              fj = makeNode(ForcedJoin);
                fj = (ForcedJoin *) lfirst(p1);
                elog(DEBUG1, "JoinRels: cast node to forcejoin");
                Bitmapset *a1 = fj->left_relids;
                Bitmapset *a2 = fj->right_relids;
                Bitmapset *a3 = rel1->relids;
                Bitmapset *a4 = rel2->relids;
               // elog(DEBUG1, "JoinRels: a1 == NULL?, %d.",a1->nwords);
                elog(DEBUG1, "JoinRels: inital Bitmapset values.");
//                elog(DEBUG1, "JoinRels: a4 %d a3.", bms_equal(a4,a3));
  //              elog(DEBUG1, "JoinRels: a1 %d a2.", bms_equal(a1,a2));
		if(bms_equal(a1,a3) && bms_equal(a2,a4)){
                        elog(DEBUG1, "JoinRels: Match found");
			joinrel->isForcedJoin = true;
			joinrel->forcedJoinAlgorithm = fj->join_algorithm;
			
                        elog(DEBUG1, "JoinRels: Populate joinrel");
                        if(joinrel->forcedJoinAlgorithm == HASH_JOIN){
                            elog(DEBUG1, "JoinRels: Forcing Hash Join.");
                        }
                        else if(joinrel->forcedJoinAlgorithm == MERGE_JOIN){
                            elog(DEBUG1, "JoinRels: Forcing Merge Join."); 
                        }else{
                            elog(DEBUG1, "JoinRels: Error."); 
                        } 
	...
	switch (sjinfo->jointype)
	{
        ...
        elog(DEBUG1,"JoinRels finish.");
	bms_free(joinrelids);
        elog(DEBUG1,"JoinRels free.");
	return joinrel;
}
