...
add_path(RelOptInfo *parent_rel, Path *new_path)
{
	 ...
	 */
         JoinPath *j_old_path = NULL;
	 JoinPath *j_new_path = NULL;
//	 j_old_path = makeNode(HashJoin);
//	 j_new_path = makeNode(HashJoin);
         elog(DEBUG1, "Pathnode: parent_rel->isForcedJoin: %d",parent_rel->isForcedJoin);
         elog(DEBUG1, "Pathnode: IsA(new_path,HashJoin): %d",new_path->pathtype == T_HashJoin);
         elog(DEBUG1, "Pathnode: parent_rel->forcedJoinAlgorithm == HASH_JOIN: %d",parent_rel->forcedJoinAlgorithm == HASH_JOIN);
         elog(DEBUG1, "Pathnode: parent_rel->forcedJoinAlgorithm == MERGE_JOIN: %d",parent_rel->forcedJoinAlgorithm == MERGE_JOIN);
         elog(DEBUG1, "Pathnode: IsA(new_path,MergeJoin): %d",new_path->pathtype == T_MergeJoin);

      /* see if the new path matches the hint, if it does, mark the path*/
	 if((parent_rel->isForcedJoin) && ((new_path->pathtype == T_HashJoin && parent_rel->forcedJoinAlgorithm == HASH_JOIN) ||\
                                     (new_path->pathtype == T_MergeJoin && parent_rel->forcedJoinAlgorithm == MERGE_JOIN)))
         {
		        j_new_path = (JoinPath *) new_path;
                if(new_path->pathtype == T_HashJoin){
                     j_new_path->isForcedJoin = true;
                     j_new_path->forcedJoinAlgorithm = HASH_JOIN;
                }
                else if(new_path->pathtype == T_MergeJoin){
                     j_new_path->isForcedJoin = true;  
                     j_new_path->forcedJoinAlgorithm = MERGE_JOIN; 
                }else{
                     elog(DEBUG1, "Pathnode: new path type error.");
                } 
                elog(DEBUG1, "Pathnode: cast new path to a joinpath");
	 }
	...
	for (p1 = list_head(parent_rel->pathlist); p1 != NULL; p1 = p1_next)
	{
		...
		p1_next = lnext(p1);
            ...
            /*CS448: if the old path is a JoinPath, create a j_old_path*/
           if(old_path->pathtype == T_HashJoin || old_path->pathtype == T_MergeJoin)
           {
                j_old_path = (JoinPath *) old_path;
                elog(DEBUG1, "Pathnode: cast old path to a joinpath");
           }

          
            
		  /*if the old path or new path were marked, skip the cost comparing process*/
           if(!j_old_path || !((j_old_path)->isForcedJoin))
           {   
               
             if(!j_new_path || !((j_new_path)->isForcedJoin))
             {
                elog(DEBUG1, "Pathnode: Normal plan");	
		/*
		...
		if (costcmp != COSTS_DIFFERENT)
		{
	      ...
	      {
	  	  	remove_old = true;
                        elog(DEBUG1, "Pathnode: Force join. new dominates");
	      } 
           }
           else
           {
        	accept_new = false;
                elog(DEBUG1, "Pathnode: Force join. Old dominates");
           }
		/*
		 * Remove current element from pathlist if dominated by new.
		 */
                 elog(DEBUG1, "Pathnode: Remove old: %d",remove_old);
		if (remove_old)
		{
        ...
        elog(DEBUG1, "Pathnode: accept_new: %d",accept_new);
