...
//	 ListCell *p1_prev;
    ...
    /* Go through the fromlist to find JoinExpr*/
    for (p1 = list_head(parse->jointree->fromlist); p1 != NULL; p1 = p1_next)
	{       elog(DEBUG1, "In planner begins.");
		if(IsA(lfirst(p1),JoinExpr)){
                   elog(DEBUG1, "Planner: it is a JoinExpr");
                   j = (JoinExpr *) lfirst(p1);
                   ForcedJoin *fj;
                   fj = makeNode(ForcedJoin);

         /* If it is a forced join expression, store hints in the fj, and append it to the forcedjoin list*/
          if(j->isForcedJoin){
                  	 fj->join_algorithm = j->forcedJoinAlgorithm;
                         if (IsA(j->larg, RangeTblRef))
			{
				int varno = ((RangeTblRef *) j->larg)->rtindex;
                                fj->left_relids = bms_add_member(fj->left_relids,varno);		
			...
			if (IsA(j->rarg, RangeTblRef))
			{
				int varno = ((RangeTblRef *) j->rarg)->rtindex;
                                 fj->right_relids = bms_add_member(fj->right_relids,varno);
				//markRTEForSelectPriv(pstate, NULL, varno, InvalidAttrNumber);
			}
...
//				markRTEForSelectPriv(pstate, NULL, varno, InvalidAttrNumber);
				...
				elog(ERROR, "unrecognized node type: %d",(int) nodeTag(j->rarg));
       		  	...
       		  	// fj->left_relids =(Bitmapset *) ((RangeTblRef *) j->larg)->rtindex;
       		  	// fj->right_relids = (Bitmapset *) ((RangeTblRef *) j->rarg)->rtindex;
	          	 glob->forcedjoins = lappend(glob->forcedjoins,fj);
                        
                  	 elog(DEBUG1, "Planner: forcejoins appended.");
          }
		}
