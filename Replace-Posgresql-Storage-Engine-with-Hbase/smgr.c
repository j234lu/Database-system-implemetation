...
static const f_smgr smgrsw[] = {
	 ...
	 * Add hba
	 */
	{hbainit, NULL, hbaclose, hbacreate, hbaexists, hbaunlink, hbaextend,
		hbaprefetch, hbaread, hbawrite, hbanblocks, hbatruncate, hbaimmedsync,
		NULL,NULL,NULL
	}
};
...
smgropen(RelFileNode rnode, BackendId backend)
{
	...
	if (!found)
	{
		...
		/* CS648:
		 * Check if this is a Hbase relation.
		 */
		if (hbaexists(reln, MAIN_FORKNUM)){
			reln->smgr_which = 1;
		} else {
			reln->smgr_which = 0;
		}
...
{   
	//return (*(smgrsw[1].smgr_exists)) (reln, forknum);
...
smgrclose(SMgrRelation reln)
{
	...
	for (forknum = 0; forknum <= MAX_FORKNUM; forknum++)
		//(*(smgrsw[1].smgr_close)) (reln, forknum);
...
//	(*(smgrsw[1].smgr_create)) (reln, forknum, isRedo);
}
		   ...
		   char *buffer, bool skipFsync)
{
	(*(smgrsw[reln->smgr_which].smgr_extend)) (reln, forknum, blocknum,
											   buffer, skipFsync);
	//(*(smgrsw[1].smgr_extend)) (reln, forknum, blocknum,
											   //buffer, skipFsync);
}
...
smgrprefetch(SMgrRelation reln, ForkNumber forknum, BlockNumber blocknum)
{
	...
	//(*(smgrsw[1].smgr_prefetch)) (reln, forknum, blocknum);
}
		 ...
		 char *buffer)
{
	...
	//(*(smgrsw[1].smgr_read)) (reln, forknum, blocknum, buffer);
}
		  ...
		  char *buffer, bool skipFsync)
{
	(*(smgrsw[reln->smgr_which].smgr_write)) (reln, forknum, blocknum,
											  buffer, skipFsync);
	//(*(smgrsw[1].smgr_write)) (reln, forknum, blocknum,
											  //buffer, skipFsync);
}
...
smgrnblocks(SMgrRelation reln, ForkNumber forknum)
{
	...
	//return (*(smgrsw[1].smgr_nblocks)) (reln, forknum);
}
...
smgrtruncate(SMgrRelation reln, ForkNumber forknum, BlockNumber nblocks)
{
	...
	//(*(smgrsw[1].smgr_truncate)) (reln, forknum, nblocks);
}
...
smgrimmedsync(SMgrRelation reln, ForkNumber forknum)
{
	...
	//(*(smgrsw[1].smgr_immedsync)) (reln, forknum);
}
