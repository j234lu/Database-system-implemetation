...
_bt_prefixKeyCompress(Relation rel, IndexTuple lowItem, IndexTuple highItem)
{
  ...
  /* CS448 - ANY VARIABLES YOU DEFINE SHOULD BE DECLARED HERE */
  ...
  if(ATTR_IS_TEXT(attr)) {
    ...
    /* YOUR SOLUTION HERE */
    rep_len = 0;
    len_low = VARSIZE_ANY_EXHDR(lowText);
    len_high = VARSIZE_ANY_EXHDR(highText);
    if(len_low < len_high){
        min = len_low;
    }else{
        min = len_high;
    ...
    for (i = 0; i < min; i++){
    	if(highp[i] == lowp[i] ||\
         (((highp[i] >= "a" && highp[i] <= "z")||(highp[i] >= "A" && highp[i] <= "Z"))\
      && ((lowp[i] >= "a" && lowp[i] <= "z")||(lowp[i] >= "A" && lowp[i] <= "Z"))\ 
      && (abs(highp[i] - lowp[i]) == 26))){
    		rep_len++;
    ...
    abv_len = rep_len + 1;
    truncate_len = len_high - abv_len;
    SET_VARSIZE_SHORT(highText,abv_len+1);
    toReturn = truncate_len;
    ...
    for (i=0; i < VARSIZE_ANY_EXHDR(highText); i++)
