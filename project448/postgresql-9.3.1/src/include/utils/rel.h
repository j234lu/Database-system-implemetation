 ...
 * CS448: also check if this is a RocksDB relation and set storage manager flag.
 */
#define RelationOpenSmgr(relation) \
	do { \
		if ((relation)->rd_smgr == NULL) \
			smgrsetowner(&((relation)->rd_smgr), smgropen((relation)->rd_node, (relation)->rd_backend)); \
			if(strncmp(RelationGetRelationName((relation)), "hba_", 4) == 0) \
			{ \
				(relation)->rd_smgr->smgr_which = 1; \
			} \
			else \
			{ \
				(relation)->rd_smgr->smgr_which = 0; \
			} \
	} while (0)
